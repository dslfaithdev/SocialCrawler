...
while (true) {
  ...
  #Fetch new id's
    ...
    try {
        ...
        print "Error fetching new posts, will sleep 5s and try again.".$e->getMessage().PHP_EOL;
        sleep(5);
        continue;
    }
    ...
    for ($i=0; $i<10; $i++) {
        ...
        try {
          ...
          //$GZout = gzencode($JSONout);
            ...
            print "Exception catched trying to push " . $e->getMessage();
            unset($e);
            get_execution_time(1);
            continue;
        }
          ...
          /* Try to handle strange errors with huge amounts of comments */
            if (strpos($e->getMessage(), "Operation timed out after") !== false) {
              /* It seems like it might be possible to retrieve if one first gets only the id. */
                try {
                    $facebook->api($url, 'GET', array('limit' => 200, 'fields' => 'id'));
                } catch (Exception $ex) {
                    unset($ex);
                }
            }
