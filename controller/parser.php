            ...
            foreach ($d['ec_likes']['data'] as $user) {
                if (isset($user['id'])) {
                    ...
                    if (empty($page_id) || empty($post_id) || empty($comment_id)) {
                        throw new Exception('Problem parsing likes on comments, recrawl is suggested.');
                    }//print_r($comments);print_r($d); exit(PHP_EOL.$page_id.", ".$post_id. ", ". $comment_id.PHP_EOL); }
                    $table["likedby"][] = [
